...
下图是目标架构。当前可运行模型预览、独立 Odin 传感器场景、整车落地接触与
ros2_control 运动仿真，以及离线误差评测。随车传感器和 C++ 单分支感知/跟踪已接入运动仿真；
有序路线选择和连续整圈状态已在 `lap_controller` 内实现；实车比赛集成和 F4 通信尚未实现；各包状态见[源码导航](../src/README_cn.md)。
...
当前模型将 `base_link` 放在后驱动轴中点；实车需核对该参考点，必要时用实测固定偏移表示评分参考点。前万向球是被动支撑，不接收驱动指令。
...
当前运动仿真由 `diff_drive_controller` 根据轮位置反馈发布 `odom -> base_link`，
`robot_state_publisher` 发布车内变换，话题为 `/sim/racer/tf` 和 `/sim/racer/tf_static`。
模拟传感器坐标系使用 `odin_sim_*` 名称；整车默认在 `/sim/racer/odin1` 输出图像、点云和 IMU。
Gazebo 世界真值独立用于验证，不参与轮式里程计，也没有发布 `world -> odom`
或 `map -> odom`。未来接入定位节点时需明确移交 TF 发布责任。
...
整圈控制器已在仿真中实现显式使能、就绪、运行、完成与故障锁存停车；实车模式仲裁仍待完成。单独底盘仿真启动只激活轮状态广播器与差速控制器，等待外部带时间戳的速度指令，
不自动发送非零速度。仿真指令超时使用仿真时间，不替代 F4 独立看门狗。
后续比赛启动流程必须拒绝不完整标定，且不能因启动程序就自动使能电机。
现有入口与验证范围见[仿真说明](../simulation/README_cn.md)。
