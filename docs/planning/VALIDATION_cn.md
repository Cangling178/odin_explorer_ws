...
## C++ 低速视觉循线早期基线 — 2026-09-14

10 个包构建、28 项 Python 测试、10 项 C++ 测试和 16 项 ROS 输入/状态检查通过。
直线 25 s、圆弧 40 s 的连续跟踪与失效停车通过，末段横向 RMS 分别约 1.19 mm、8.04 mm。
比赛参考图初始直段另外通过 3 s 短距离检查。完整指标、开发失败记录、参数与复现入口见
[循线实现与验证](../../simulation/LINE_FOLLOWING_cn.md)。不是实车或完整路线验收。

## 独立场景验收与提交前复核 — 2026-09-14

冻结版本完成42/42次跟踪及12/12次故障仿真验收，覆盖七种独立场景；详见
[交付结果](../../experiments/isolated_line/RESULTS_cn.md)。这不包含完整比赛路线和实车验收。
冻结后比赛地图生成与相关说明已更新；感知、控制和评测算法未变，提交前仅另清理了两处空白字符。
比赛地图现在默认2倍（4×3 m），黑线约21.2 mm，默认出生点随地图缩放；右上角示例保留对应位置和朝向。

本次提交前重新运行：结构/语言配对/本地链接检查、36项Python测试、11个ROS包构建，
以及23项colcon测试结果（21个C++用例加两个套件结果），均通过。
`validate_line_controller.py`的17项合成ROS检查和`validate_arming_clock.py`的20次重复使能检查通过。
地图测试覆盖2、2.5、4倍下的线宽与连通性，以及默认出生位置、传感器和物理模型保持一致。
本次未重跑54次Gazebo验收矩阵，也未新增完整比赛地图循线验收。
