...
点云先用设备外参变换到车体坐标中调用 PCL 过滤，然后变回雷达坐标输出。这样 Nav2 清除射线仍从真实雷达原点发出。原采集时间戳保持不变。标记高度和清除回波分别处理，避免因删除地面或远处高回波导致旧障碍无法清除。
...
- 开启里程计、原始点云和设备 TF。
- `use_host_ros_time` 保留厂商配置文件的值（当前为 `0`），不再强制改成 `2`。原强制覆盖未经过实机验证，现已撤销；时间戳与 ROS 时间是否一致仍需结合实际输出检查。
...
以上命令仅说明 `0001` 配置参数补丁。后续还增加了 `0002` SDK 符号隔离和 `0003` RGB 失败诊断；完整部署按[部署与构建](02_deployment.md)依次应用所有补丁。如果复制的是已经修补的厂商源码，不要重复应用补丁。应用后需要重新编译厂商包；补丁不会覆盖用户原有的 `control_command.yaml` 内容。
...
临时验证脚本和临时场景数据在验证后删除，没有加入交付源码。随后项目整理已删除原有建图测试和纯模拟导航测试；本页保留历史验证结果，不再保留测试代码。

以上证明本次软件接口与 Nav2 链路在受控输入下能够工作，不等同于 Jetson 实物运行、ODIN 实测视场或跨机网络已验收。本次没有可用的 Jetson 实机连接，因此 ARM 环境依赖、设备时钟对齐质量、实际安装外参、两份实验室地图的对齐及网络表现，需要按[操作建议](04_navigation.md#建议的操作顺序)现场核对。
