...
2026-09-16 更新：[完整地图连续整圈](COMPETITION_LAP_cn.md)已通过初始验收、三次同配置重复及一次 0.10 m/s 速度试验：185/185检查点，中途不停。采用C++预录路线跟踪与车载视觉校正；实车及正式比赛验收待完成。
...
详见[赛道启动、比例与验证](COMPETITION_COURSE_cn.md)。这是图片复刻版；已有单分支低速循线和选定完整路线的连续整圈实现。
...
通过已知目标投影与运动响应检查。已验证独立直线、曲线、角点和故障场景，并完成选定地图的连续整圈。下一步验证真实安装下的黑线视野、标定与设备接入。
...
并检查初始直段黑线投影；完整路线算法闭环现已单独验收；全赛道遮挡和真实光照变化仍需专项测试。
