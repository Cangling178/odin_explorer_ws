...
范围：现有仿真车辆，在直线、左右圆弧、S 弯和单个左右直角场景中使用车载 FishPoly 图像跟踪。
...
上面的S弯示例显式指定2倍比例。
车辆、相机内参与循线算法参数不变。`scale: 1.0`可恢复原比例；显式`spawn_x/y`使用缩放后的世界米制坐标，
不会再次乘比例，默认起点则随地图缩放。GUI观察视点及可选俯视相机位置同步调整，原地图资源文件不变。
完整地图的紧凑S弯尚未按独立S弯的验收矩阵验证，放置成功不代表可以连续通过。

## 感知实现

1. 精确匹配图像和 CameraInfo 的采集时间戳，校验 FishPoly 六项畸变参数、内参及相机帧。
...
`line_geometry.hpp` 中的算法可脱离 ROS 测试。`line_offline` 可重放保存的默认米制地面灰度 PNG，
输出骨架、掩膜、角点/出口叠加图和有序路径 JSON。固定阈值只对应当前仿真光照；置信度是工程质量分，并非统计概率。
...
根据速度、路径曲率和可见距离调节前视，保持 0.28–0.45 m 的硬边界；前视还必须位于实际可见起点
...
| 参数 | 默认值 |
| --- | --- |
| 米制地面网格 | x=0.25–0.75 m，y=±0.30 m，5 mm 网格 |
| 地面平面 | base_link 下方 33.25 mm；平地近似 |
| 黑色阈值 / 线厚度 | 灰度 65 / 8–50 mm |
| 最小路径长度 / 起点横向范围 | 0.18 m / ±0.12 m |
| 正常速度 / 前视范围 | ≤0.05 m/s / 0.28–0.45 m |
...
| `perception_status` | 检测原因、置信度和截断原因 |
...
存在栅格误差。车辆和感知、控制代码及参数均不变，独立测试场景不受影响。
关闭旧仿真后重新启动以加载新纹理；本次验证了纹理线宽、连通性和生成资源，未重新验收比赛地图循线。
