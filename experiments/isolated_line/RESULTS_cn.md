...
随后地图波浪段开发又调整了感知和控制；本页结果只对应原冻结版本。`results.json`中的源码一致性字段为冻结时记录，不是对当前工作区的实时检查。本结论针对现有车辆的 Gazebo 仿真及工程测试走廊，
不代表实车性能或正式比赛合规性。
...
本页记录原冻结版本；后续[地图波浪段优化](../../simulation/COMPETITION_WAVES_cn.md)已修改感知和控制，不能将本页54次结果直接视为新版本的重复验收。
