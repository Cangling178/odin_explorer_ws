...
def load_sensor_config(path):
    ...
    required = {
        "camera": {"width", "height", "rate_hz", "horizontal_fov_rad", "near_m", "far_m"},
    ...
    for name, keys in required.items():
        ...
        for key, value in values.items():
            ...
            if key in {"width", "height", "horizontal_samples", "vertical_samples"} and (int(value) != value or value < 2):
    ...
    if config["camera"]["horizontal_fov_rad"] >= math.pi:
        raise ValueError("Pinhole camera FOV must be less than pi")
...
def add_odin_sensors(link, robot, parent, config, namespace):
    ...
    for name, kind, frame, library in (
        ...
        ("camera", "camera", "odin_sim_camera_optical", "camera"),
        ...
        if name == "camera":
            ...
            element(camera, "horizontal_fov", params["horizontal_fov_rad"])
            image = element(camera, "image")
            for key in ("width", "height"):
                element(image, key, int(params[key]))
        ...
        plugin = element(sensor, "plugin", name="odin_"+name, filename=f"libgazebo_ros_{library}.so")
        ros = element(plugin, "ros")
        element(ros, "namespace", namespace)
        element(plugin, "frame_name", frame)
        if name == "camera":
            element(plugin, "camera_name", "rgb")
            element(ros, "remapping", f"rgb/image_raw:={namespace}/image")
            element(ros, "remapping", f"rgb/camera_info:={namespace}/camera_info")
        else:
