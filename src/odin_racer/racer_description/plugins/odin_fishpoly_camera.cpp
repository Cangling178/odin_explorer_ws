// 将 Gazebo Classic 的等距广角渲染重采样为 Odin FishPoly 图像。
// 模型参考：manifoldsdk/odin_ros_driver/include/polynomial_camera.hpp。
// 几何模型与自定义 CameraInfo 约定需和 fishpoly.py 同步。
...
class FishPolyCamera : public gazebo::SensorPlugin {
  ...
  void Load(gazebo::sensors::SensorPtr sensor, sdf::ElementPtr sdf) override {
    ...
    try {
      ...
      if (camera->ImageWidth() != camera->ImageHeight() ||
        ...
        throw std::runtime_error("Expected square 180 degree intermediate rendering, got " +
                                 std::to_string(camera->ImageWidth()) + "x" +
                                 std::to_string(camera->ImageHeight()) +
                                 " hfov=" + std::to_string(camera->HFOV().Radian()));
      ...
      for (unsigned v = 0; v < info_.height; ++v) {
        for (unsigned u = 0; u < info_.width; ++u) {
          ...
          if (radius >= Radius(M_PI / 2))
            throw std::runtime_error("Calibration exceeds forward hemisphere");
          double lo = 0, hi = M_PI / 2;
          for (int i = 0; i < 48; ++i) {
            const double mid = (lo + hi) / 2;
            if (Radius(mid) < radius)
              lo = mid;
            else
              hi = mid;
          }
          const double theta = (lo + hi) / 2;
          // 避开内置广角着色器在视场边界处的渐隐区域。
          if (theta > M_PI / 2 - 0.04)
            throw std::runtime_error("Calibration too close to render cutoff");
          const double scale = radius > 1e-15 ? focal * theta / radius : focal;
          // Gazebo 片元中心为 (i + 0.5) / size，OpenCV 像素中心为整数坐标。
      ...
      info_pub_ =
          node_->create_publisher<sensor_msgs::msg::CameraInfo>("camera_info", rclcpp::QoS(5));
      connection_ = camera->ConnectNewImageFrame(std::bind(&FishPolyCamera::OnImage,
                                                           this,
                                                           std::placeholders::_1,
                                                           std::placeholders::_2,
                                                           std::placeholders::_3,
      ...
      RCLCPP_INFO(node_->get_logger(),
                  "FishPoly ready: %ux%u, full calibrated frame",
                  info_.width,
                  info_.height);
 ...
 private:
  ...
  void OnImage(const unsigned char *data,
               unsigned width,
               unsigned height,
               unsigned depth,
               const std::string &) {
    ...
    if (last_frame_stamp_ > 0 && stamp - last_frame_stamp_ > .15)
      RCLCPP_WARN(node_->get_logger(),
                  "FishPoly source frame gap %.3f simulated seconds",
    ...
    if (elapsed > .1)
      RCLCPP_WARN(node_->get_logger(), "FishPoly frame processing/publish took %.3f s", elapsed);
  }
