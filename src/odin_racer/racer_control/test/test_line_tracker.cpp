...
TEST(Tracker, StraightAndSignedCorrection) {
  ...
  ASSERT_TRUE(straight.valid); EXPECT_DOUBLE_EQ(straight.v,.05); EXPECT_DOUBLE_EQ(straight.w,0);
  auto left=pursuit({{.2,.04},{.6,.04}},.4,.05,.5);
  auto right=pursuit({{.2,-.04},{.6,-.04}},.4,.05,.5);
  EXPECT_GT(left.w,0); EXPECT_NEAR(left.w,-right.w,1e-12);
}
TEST(Tracker, CircleCurvatureAndYawBound) {
  std::vector<Point> path;
  for(int i=0;i<=100;i++) { double a=i*.01; path.push_back({.8*std::sin(a), .8*(1-std::cos(a))}); }
  auto command=pursuit(path,.4,.05,.5);
  ASSERT_TRUE(command.valid); EXPECT_NEAR(command.w/command.v,1/.8,.001);
  auto limited=pursuit(path,.4,.2,.1);
  ASSERT_TRUE(limited.valid); EXPECT_LE(std::abs(limited.w),.1+1e-12); EXPECT_LT(limited.v,.2);
}
TEST(Tracker, InvalidAndInsufficientPaths) {
  ...
  EXPECT_FALSE(pursuit({{.2,0},{.6,std::numeric_limits<double>::quiet_NaN()}},.4,.05,.5).valid);
}
TEST(Tracker, FreshnessAndAcceleration) {
  EXPECT_TRUE(fresh(10,9.9,.35)); EXPECT_FALSE(fresh(10,9,.35));
  EXPECT_FALSE(fresh(10,11,.35)); EXPECT_FALSE(fresh(10,0,.35));
...
TEST(Tracker, AdaptiveBoundsAndTargetJumpGuard) {
  std::vector<Point> path;for(int i=0;i<100;++i)path.push_back({.25+i*.005,.02});
  auto c=adaptive_pursuit(path,.4,.28,.45,.05,.03,.5,.15);
  ASSERT_TRUE(c.valid);EXPECT_GE(c.lookahead,.28);EXPECT_LE(c.lookahead,.45);
  Point old{.70,.02};EXPECT_FALSE(adaptive_pursuit(path,.4,.28,.45,.05,.03,.5,.15,&old).valid);
  path.resize(5);EXPECT_FALSE(adaptive_pursuit(path,.4,.28,.45,.05,.03,.5,.15).valid);
}
TEST(Corner, StableEvidenceStopTurnAndDistinctReacquisitionFrames) {
  CornerTurn turn;std::string error;
  ...
  EXPECT_DOUBLE_EQ(stop.v,0);EXPECT_EQ(turn.phase,CornerTurn::Phase::STOP);
  ...
  EXPECT_GT(rotate.w,0);EXPECT_DOUBLE_EQ(rotate.v,0);
  turn.update(13,{.485,0},M_PI/2,0,0,false,13,error);
  EXPECT_EQ(turn.phase,CornerTurn::Phase::REACQUIRE);
  for(int i=0;i<10;++i)turn.update(13.1,{.485,0},M_PI/2,0,0,true,13.1,error);
  EXPECT_EQ(turn.phase,CornerTurn::Phase::REACQUIRE);
  turn.update(13.2,{.485,0},M_PI/2,0,0,true,13.2,error);
  turn.update(13.3,{.485,0},M_PI/2,0,0,true,13.3,error);
  EXPECT_EQ(turn.phase,CornerTurn::Phase::TRACK);EXPECT_TRUE(turn.consumed);EXPECT_TRUE(error.empty());
}
TEST(Corner, MemoryAndMissingExitFailClosedBothDirections) {
  for(int sign:{-1,1}) {
    CornerTurn turn;std::string error;
    for(int i=0;i<3;++i)turn.observe(1+i*.1,{.3,0},sign*M_PI/2,0,{0,0},0);
...
TEST(Corner, DistanceAngleAndExitTimeoutAreIndependentStops) {
  auto setup=[]() {
    CornerTurn t;
    for(int i=0;i<3;++i)t.observe(1+i*.1,{.3,0},M_PI/2,0,{0,0},0);
    std::string e;t.update(1.2,{0,0},0,0,0,false,1.2,e);return t;
  };
  {
    auto t=setup();std::string e;
    t.update(2,{.46,0},0,0,0,false,2,e);EXPECT_EQ(e,"corner_memory_limit");
  }
  {
    auto t=setup();std::string e;
    t.update(2,{0,0},2.,0,0,false,2,e);EXPECT_EQ(e,"corner_memory_limit");
  }
  {
    auto t=setup();std::string e;
    ...
    EXPECT_FALSE(command.valid);EXPECT_EQ(e,"exit_not_reacquired");
  }
}
...
TEST(Tracker, SharedSegmentEndpointSurvivesFloatingPointMotionCompensation) {
  for(int k=0;k<1000;++k) {
    ...
    for(int i=0;i<80;++i) {double x=.31+i*.005-shift;path.push_back({x*std::cos(yaw),x*std::sin(yaw)});}
