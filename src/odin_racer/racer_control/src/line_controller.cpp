  ...
  LineController() : Node("line_controller"), buffer_(get_clock()), listener_(buffer_)
  {
    ...
    for (double v : {speed_,
      ...
      if (!std::isfinite(v) || v <= 0)
        throw std::invalid_argument("Invalid positive controller parameter");
    if (speed_ > .2 || max_yaw_ > 1 || acceleration_ > .3 || yaw_acceleration_ > 1.5 ||
        min_lookahead_ < .10 || min_lookahead_ > lookahead_ || lookahead_ > max_lookahead_ ||
        max_lookahead_ > .7 || base_.empty() || odom_frame_.empty())
      throw std::invalid_argument("Controller parameters exceed baseline limits");

    // 弯角记忆同时受时间、累计位移和累计转角约束。
    ...
    for (double v : {corner_.config.memory_time,
                     corner_.config.memory_distance,
                     corner_.config.memory_angle,
                     corner_.config.approach_speed,
                     corner_.config.turn_rate,
                     corner_.config.stop_distance})
      if (!std::isfinite(v) || v <= 0)
        throw std::invalid_argument("Invalid corner parameter");
    if (corner_.config.memory_time > 20 || corner_.config.memory_distance > .5 ||
        corner_.config.memory_angle > 2.2 || corner_.config.approach_speed > speed_ ||
        corner_.config.turn_rate > max_yaw_ || corner_.config.stop_distance > .10)
      throw std::invalid_argument("Corner parameters exceed bounded low-speed limits");

    // ROS 通信：观测先入队，等待采集时刻的 TF，再由控制周期统一消费。
    command_pub_ = create_publisher<geometry_msgs::msg::TwistStamped>("cmd_vel", 1);
    state_pub_ = create_publisher<std_msgs::msg::String>("tracking_status",
                                                         rclcpp::QoS(1).transient_local());
    debug_pub_ = create_publisher<std_msgs::msg::String>("control_debug", 1);
    observation_sub_ = create_subscription<racer_interfaces::msg::LineObservation>(
        "observation",
        1,
        [this](racer_interfaces::msg::LineObservation::ConstSharedPtr observation) {
          ...
          if (stamp.nanoseconds() > seen_stamp_ &&
              (pending_.empty() || stamp > rclcpp::Time(pending_.back()->path.header.stamp))) {
            pending_.push_back(observation);
            if (pending_.size() > 5) {
              pending_.clear();
              image_valid_ = false;
              stop("observation_queue_overflow");
            }
          }
        });
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
        "odom", rclcpp::SensorDataQoS(), [this](nav_msgs::msg::Odometry::ConstSharedPtr odom) {
          ...
          if (odom->header.frame_id != odom_frame_ || odom->child_frame_id != base_ ||
            ...
            odom_.reset();
            stop("invalid_odometry");
            return;
          }
          // 时钟与里程计通过独立 DDS 流到达：忽略迟到、未来或乱序消息，
          // 但保留仍新鲜的上一帧；compute() 分别按 ROS 时间和单调时钟检查超时。
          const auto stamp = rclcpp::Time(odom->header.stamp);
          if (!racer_control::fresh(now().seconds(), stamp.seconds(), odom_age_) ||
              (odom_ && stamp <= rclcpp::Time(odom_->header.stamp)))
            return;
          odom_ = odom;
          odom_wall_ = std::chrono::steady_clock::now();
        });
        ...
        [this](const std_srvs::srv::SetBool::Request::SharedPtr request,
          ...
          if (!request->data) {
            ...
            if (!command.valid)
              publish_zero();
            if (command.valid) {
              // 加速计时从使能时刻开始，避免沿用未使能时的控制周期。
              // 仿真时钟可能在就绪检查与零速发布之间跳变。
              last_tick_ = now().seconds();
              armed_ = true;
              state_ = "RUNNING";
            } else
              state_ = "STOPPED: " + error;
          }
 ...
 private:
  ...
  void publish(double v, double w, bool tick_command = false) {
    ...
    command.header.stamp =
        tick_command
            ? rclcpp::Time(static_cast<int64_t>(std::llround(last_tick_ * 1e9)), RCL_ROS_TIME)
            : now();
    command.header.frame_id = base_;
    command.twist.linear.x = v;
    command.twist.angular.z = w;
    command_pub_->publish(command);
  }

  void publish_zero(bool tick_command = false) {
    last_v_ = last_w_ = 0;
    publish(0, 0, tick_command);
  }
  ...
  void stop(const std::string &reason) {
    ...
    if (armed_) {
      armed_ = false;
      state_ = "STOPPED: " + reason;
      publish_state();
    }
    publish_zero();
  }

  // 校验观测并转换到 odom；无效观测清除路径并触发停车。
  void receive(const racer_interfaces::msg::LineObservation &observation) {
    ...
    if (stamp.nanoseconds() > 0 && stamp.nanoseconds() <= seen_stamp_)
      return;
    auto invalidate = [this](const std::string &why) {
      points_.clear();
      image_valid_ = false;
      stop(why);
    };
    if (path.header.frame_id != base_ ||
        !racer_control::fresh(now().seconds(), stamp.seconds(), path_age_) ||
        !observation.image_valid || !std::isfinite(observation.confidence) ||
        observation.exits > 1) {
      invalidate("invalid_observation: " + observation.reason);
      return;
    }
    seen_stamp_ = stamp.nanoseconds();
    try {
      auto tf =
          buffer_.lookupTransform(odom_frame_, base_, stamp, rclcpp::Duration::from_seconds(0))
              .transform;
      ...
      if (!std::isfinite(norm) || std::abs(norm - 1) > .01 || !std::isfinite(tf.translation.x) ||
        ...
        invalidate("malformed_observation_transform");
        return;
      }
      ...
      for (const auto &pose : path.poses) {
        ...
        if (pose.header.frame_id != base_ || pose.header.stamp != path.header.stamp ||
            !std::isfinite(p.z)) {
          invalidate("malformed_path_header");
          return;
        }
        local.push_back({p.x, p.y});
        auto shifted = transform * tf2::Vector3(p.x, p.y, p.z);
        global.push_back({shifted.x(), shifted.y()});
      }
      if (observation.path_valid &&
          (!racer_control::ordered_path(local) || observation.confidence < .5)) {
        invalidate("malformed_path");
        return;
      }
      if (!observation.path_valid && !path.poses.empty()) {
        invalidate("inconsistent_observation");
        return;
      }
      image_valid_ = true;
      health_stamp_ = stamp.nanoseconds();
      path_wall_ = std::chrono::steady_clock::now();
      if (observation.path_valid) {
        bool accepted = true;
        points_ =
            curve_align_ ? global : racer_control::merge_observed_path(points_, global, &accepted);
        if (accepted)
          path_stamp_ = stamp.nanoseconds();
      } else {
        const bool clipped = observation.reason == "no_near_seed" ||
                             observation.reason == "insufficient_visible_length";
        if (!clipped ||
            (!corner_.active() &&
             !racer_control::fresh(now().seconds(), path_stamp_ * 1e-9, curve_memory_))) {
          invalidate("line_lost: " + observation.reason);
          return;
        }
      }
      // 只有单一出口且置信度足够时，才更新弯角位置与入/出弯方向。
      if (observation.corner_valid && observation.path_valid && observation.confidence >= .6 &&
          observation.exits == 1) {
        const auto &c = observation.corner;
        const auto &e = observation.exit_direction;
        if (!std::isfinite(c.x) || !std::isfinite(c.y) || !std::isfinite(e.x) ||
            !std::isfinite(e.y) || std::abs(std::hypot(e.x, e.y) - 1) > .05) {
          invalidate("invalid_corner");
          return;
        }
        size_t index = 0;
        double best = 1e9;
        for (size_t i = 0; i < local.size(); ++i) {
          double d = racer_control::distance(local[i], {c.x, c.y});
          if (d < best) {
            best = d;
            index = i;
          }
        ...
        if (best > .02 || index < 2) {
          invalidate("corner_off_path");
          return;
        }
        size_t near = index - 1;
        while (near > 0 && racer_control::distance(local[near], local[index]) < .035)
          --near;
        size_t before = std::min(size_t(3), near);
        // 用足够长的入弯直线拟合方向，排除被裁切的起点和骨架圆角；
        // 避免仅凭两个像素估计航向。
        if (racer_control::distance(local[before], local[near]) < .12)
          return;
        double mx = 0, my = 0;
        for (size_t i = before; i <= near; ++i) {
          mx += local[i].x;
          my += local[i].y;
        }
        double n = near - before + 1;
        mx /= n;
        my /= n;
        double xx = 0, xy = 0, yy = 0;
        for (size_t i = before; i <= near; ++i) {
          double x = local[i].x - mx, y = local[i].y - my;
          xx += x * x;
          xy += x * y;
          yy += y * y;
        }
        ...
        double residual = 0;
        for (size_t i = before; i <= near; ++i) {
          double d = -(local[i].x - mx) * std::sin(fitted) + (local[i].y - my) * std::cos(fitted);
          residual += d * d;
        }
        if (std::sqrt(residual / n) > .008)
          return;
        ...
        corner_.observe(stamp.seconds(),
                        {world_corner.x(), world_corner.y()},
                        std::atan2(e.y, e.x) + yaw,
                        incoming,
                        {tf.translation.x, tf.translation.y},
                        yaw);
      } else
        corner_.no_corner(stamp.seconds());
    } catch (const std::exception &) {
      invalidate("missing_observation_transform");
    }
  }

  // 先检查数据健康，再依次处理弯道对齐、直角转弯和常规自适应跟踪。
  racer_control::Command compute(std::string &error) {
    const double time = now().seconds();
    const auto wall = std::chrono::steady_clock::now();
    // ROS 时间检查采集新鲜度；单调时钟检测仿真暂停或消息停止。
    if (!image_valid_ || !racer_control::fresh(time, health_stamp_ * 1e-9, path_age_) ||
        std::chrono::duration<double>(wall - path_wall_).count() > wall_age_) {
      error = !pending_.empty() ? "observation_transform_timeout" : "image_timeout_or_invalid";
      return {};
    }
    if (!odom_ ||
        !racer_control::fresh(time, rclcpp::Time(odom_->header.stamp).seconds(), odom_age_) ||
        std::chrono::duration<double>(wall - odom_wall_).count() > wall_age_) {
      error = "odometry_timeout";
      return {};
    }
    ...
    std::vector<racer_control::Point> local;
    for (auto point : points_) {
      auto shifted = inverse * tf2::Vector3(point.x, point.y, p.z);
      local.push_back({shifted.x(), shifted.y()});
    }
    // 移除已经驶过的点，保留相机盲区内此前实测的近端路径，
    // 使弯道跟踪仍可使用较短的前视距离。
    size_t trim = 0;
    while (trim < local.size() && local[trim].x < .06)
      ++trim;
    local.erase(local.begin(), local.begin() + trim);
    points_.erase(points_.begin(), points_.begin() + trim);
    bool valid_path =
        !local.empty() &&
        racer_control::fresh(time, path_stamp_ * 1e-9, armed_ ? curve_memory_ : path_age_);
    if (curve_align_) {
      // 先原地转向最后实测的出口切线；收到新的有效视觉路径后再前进。
      // 用于处理驶出前向视野的紧弯，不外推未观测的线路。
      auto candidate = racer_control::adaptive_pursuit(
          local, lookahead_, min_lookahead_, max_lookahead_, speed_, 0., max_yaw_, acceleration_);
      if (path_stamp_ * 1e-9 > curve_align_stamp_ && valid_path && candidate.valid) {
        curve_align_ = false;
        target_valid_ = false;
      } else {
        if (time - curve_align_stamp_ > 5.) {
          error = "curve_exit_not_reacquired";
          return {};
        }
    ...
    if (armed_) {
      bool exit_evidence = valid_path &&
                           racer_control::fresh(time, path_stamp_ * 1e-9, path_age_) &&
                           local.size() > 8 && std::abs(local.front().y) < .10;
      if (exit_evidence) {
        auto a = local.front(), b = local.back();
        exit_evidence =
            std::abs(std::atan2(b.y - a.y, b.x - a.x)) < .25 && racer_control::distance(a, b) > .20;
      }
      auto command = corner_.update(time,
                                    {p.x, p.y},
                                    yaw,
    ...
    if (!valid_path) {
      error = "path_timeout_or_invalid";
      return {};
    }
    racer_control::Point previous;
    if (target_valid_) {
      auto a = inverse * tf2::Vector3(target_.x, target_.y, p.z);
      previous = {a.x(), a.y()};
    }
    auto command = racer_control::adaptive_pursuit(local,
                                                   lookahead_,
                                                   min_lookahead_,
                                                   max_lookahead_,
                                                   speed_,
                                                   ...
                                                   max_yaw_,
                                                   acceleration_,
                                                   target_valid_ ? &previous : nullptr);
    ...
    if (!command.valid || short_memory) {
      if (armed_ && !corner_.active() && local.size() > 8) {
        ...
        while (before > 0 && racer_control::distance(local[before], local.back()) < .06)
          --before;
        auto a = local[before], b = local.back();
        double heading = std::atan2(b.y - a.y, b.x - a.x);
        if (racer_control::distance(a, b) > .04 && std::abs(heading) > .25 &&
            std::abs(heading) < 1.8) {
          curve_align_ = true;
          curve_align_stamp_ = time;
          curve_exit_yaw_ = yaw + heading;
          target_valid_ = false;
    ...
    if (command.valid && armed_) {
      auto a = transform * tf2::Vector3(command.target.x, command.target.y, 0);
      target_ = {a.x(), a.y()};
      target_valid_ = true;
    }
    return command;
  ...
  void debug(const racer_control::Command &command) {
    ...
    out << "{\"state\":\"" << state_
        << "\",\"observation_age\":" << now().seconds() - health_stamp_ * 1e-9
        << ",\"path_age\":" << now().seconds() - path_stamp_ * 1e-9
        << ",\"lookahead\":" << command.lookahead << ",\"target\":[" << command.target.x << ","
        << command.target.y << "]"
        << ",\"corner_odom\":[" << corner_.corner.x << "," << corner_.corner.y << "]"
        << ",\"incoming_yaw_odom\":" << corner_.incoming_yaw
        << ",\"exit_yaw_odom\":" << corner_.exit_yaw
        << ",\"corner_age\":" << now().seconds() - corner_.memory_stamp
  ...
  void tick() {
    // 图像与 TF 可能乱序到达：等待变换时不阻塞控制定时器。
    // 只有成功变换的观测才刷新有效时间；缺失 TF 不延长原图像的截止时间。
    while (!pending_.empty()) {
      const auto &header = pending_.front()->path.header;
      ...
      if (!pending_.front()->image_valid || header.frame_id != base_ ||
        ...
        auto observation = pending_.front();
        pending_.pop_front();
        receive(*observation);
      } else
        break;
    ...
    const double time = now().seconds();
    double dt = time - last_tick_;
    last_tick_ = time;
    if (dt < 0) {
      points_.clear();
      odom_.reset();
      path_stamp_ = seen_stamp_ = health_stamp_ = 0;
      pending_.clear();
      stop("clock_reset");
    }
    std::string error;
    auto desired = compute(error);
    if (!armed_) {
      if (state_.rfind("STOPPED", 0) != 0)
        state_ = desired.valid ? "READY" : "DISARMED: " + error;
      publish_zero(true);
      publish_state();
      debug(desired);
      return;
    ...
    state_ = curve_align_ ? "CURVE_ALIGN" : corner_.name();
    publish_state();
    publish(last_v_, last_w_, true);
    debug(desired);
  }

  // TF 与控制状态；points_ 和 target_ 存在 odom 坐标系中。
  tf2_ros::Buffer buffer_;
  tf2_ros::TransformListener listener_;
  double curve_memory_, curve_align_stamp_ = 0., curve_exit_yaw_ = 0.;
  bool curve_align_ = false;
  double speed_, lookahead_, max_yaw_, acceleration_, yaw_acceleration_, path_age_, odom_age_,
      wall_age_;
  double last_v_ = 0, last_w_ = 0, last_tick_ = 0;
  // ROS 时间戳以纳秒存储；*_wall_ 使用单调时钟，仅用于看门狗。
