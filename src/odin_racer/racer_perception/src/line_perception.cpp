...
class LinePerception : public rclcpp::Node {
 public:
  LinePerception() : Node("line_perception"), buffer_(get_clock()), listener_(buffer_) {
    ...
    for (double v :
         {threshold_, min_width_, max_width_, min_length_, seed_limit_, min_confidence_, max_age_})
      if (!std::isfinite(v) || v <= 0)
        throw std::invalid_argument("Invalid detector parameter");
    if (threshold_ >= 255 || max_width_ <= min_width_ || min_confidence_ > 1 ||
        min_length_ > grid_.far_x - grid_.near_x || frame_.empty())
      throw std::invalid_argument("Invalid detector range");
    ...
    // 按采集时间戳精确匹配图像与内参，限制队列长度以控制图像内存占用。
    info_sub_ = create_subscription<sensor_msgs::msg::CameraInfo>(
        "camera_info", 5, [this](sensor_msgs::msg::CameraInfo::ConstSharedPtr info) {
 ...
 private:
  // 无效图像发布空路径并撤销种子提示，防止下游继续使用旧视觉结果。
  ...
  void try_process() {
    ...
    // 独立 DDS 流可能先送达下一帧图像，再送达上一帧内参。
    // 优先处理最新的完整配对；新帧缺少内参时仍允许处理较早的完整配对。
    for (auto it = images_.rbegin(); it != images_.rend() && !info; ++it) {
      for (const auto &candidate : infos_)
        if (candidate->header.stamp == (*it)->header.stamp) {
          image = *it;
          info = candidate;
          break;
        }
    ...
    try {
      ...
      racer_perception::FishPoly camera{info->k, info->d};
      camera.validate();
      auto transform = buffer_
                           .lookupTransform(image->header.frame_id,
                                            frame_,
                                            image->header.stamp,
                                            rclcpp::Duration::from_seconds(.04))
                           .transform;
      tf2::Transform camera_from_base(
          tf2::Quaternion(transform.rotation.x,
                          transform.rotation.y,
                          transform.rotation.z,
                          transform.rotation.w),
          tf2::Vector3(transform.translation.x, transform.translation.y, transform.translation.z));
      // 地面点 → 采集时刻的相机坐标 → 鱼眼像素，构造逆向重采样映射。
      ...
      cv::meanStdDev(source->image, mean, deviation);
      if (deviation[0] < 1.)
        throw std::runtime_error("blank_or_uniform_image");
      // 通过 odom 将上一帧种子补偿到当前车体坐标；位置含平移，方向仅旋转。
      racer_perception::SeedHint hint;
      if (hint_.valid && (rclcpp::Time(image->header.stamp) - hint_stamp_).seconds() < .25) {
        try {
          auto shift =
              buffer_.lookupTransform(frame_, image->header.stamp, frame_, hint_stamp_, "odom")
                  .transform;
          tf2::Transform motion(
              tf2::Quaternion(
                  shift.rotation.x, shift.rotation.y, shift.rotation.z, shift.rotation.w),
      ...
      cv::remap(source->image,
                ...
                cv::BORDER_CONSTANT,
                cv::Scalar(255));
      auto detection = racer_perception::detect(ground,
                                                visible,
                                                grid_,
                                                threshold_,
                                                min_width_,
                                                ...
                                                seed_limit_,
                                                hint);
      if (detection.confidence < min_confidence_ && detection.valid())
        detection.reason = "low_confidence";
      nav_msgs::msg::Path path;
      path.header = image->header;
      path.header.frame_id = frame_;
      if (detection.valid())
        for (const auto &p : detection.points) {
          ...
          pose.pose.position.z = grid_.ground_z;
          pose.pose.orientation.w = 1;
          path.poses.push_back(pose);
        }
      path_pub_->publish(path);
      // 图像可用与路径可用分别标记，供控制器区分短暂视野裁切和相机故障。
      racer_interfaces::msg::LineObservation observation;
      observation.path = path;
      observation.image_valid = true;
      observation.path_valid = detection.valid();
      observation.confidence = detection.confidence;
      observation.reason = detection.reason;
      observation.exits = detection.exits;
      observation.corner_valid =
          detection.valid() && detection.corner_valid && detection.exits == 1;
      observation.corner.x = detection.corner.x;
      observation.corner.y = detection.corner.y;
      observation.exit_direction.x = detection.exit_direction.x;
      observation.exit_direction.y = detection.exit_direction.y;
      observation_pub_->publish(observation);
      hint_.valid = detection.valid();
      if (hint_.valid) {
        hint_.point = detection.points.front();
        auto direction =
            detection.points[std::min(size_t(8), detection.points.size() - 1)] - hint_.point;
        hint_.direction = direction / cv::norm(direction);
        hint_stamp_ = rclcpp::Time(image->header.stamp);
      }
      if (mask_pub_->get_subscription_count())
        mask_pub_->publish(
            *cv_bridge::CvImage(image->header, "mono8", detection.mask).toImageMsg());
      if (gray_pub_->get_subscription_count())
        gray_pub_->publish(*cv_bridge::CvImage(image->header, "mono8", ground).toImageMsg());
      std_msgs::msg::String status;
      status.data = detection.reason + " confidence=" + std::to_string(detection.confidence) +
                    " end=" + detection.end_reason;
      status.data += " processing_ms=" +
      ...
      if (debug_pub_->get_subscription_count()) {
        cv::Mat debug;
        cv::cvtColor(ground, debug, cv::COLOR_GRAY2BGR);
        for (const auto &p : detection.points)
          cv::circle(debug,
                     {static_cast<int>((grid_.half_width - p.y) / grid_.step),
                      static_cast<int>((p.x - grid_.near_x) / grid_.step)},
                     1,
                     {0, 0, 255},
                     -1);
        if (detection.corner_valid) {
          ...
          cv::arrowedLine(
              debug,
              corner,
              corner + cv::Point(-detection.exit_direction.y * 15, detection.exit_direction.x * 15),
              {0, 255, 0},
              1);
        }
      ...
      publish_invalid(image->header, e.what());
    }
  }
...
int main(int argc, char **argv) {
  ...
  try {
    rclcpp::spin(std::make_shared<LinePerception>());
  } catch (const std::exception &e) {
    std::cerr << e.what() << '\n';
    rclcpp::shutdown();
    return 1;
