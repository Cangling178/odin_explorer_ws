...
TEST(FishPoly, VendorConventionAndAxis) {
  ...
  EXPECT_DOUBLE_EQ(axis.x, 800);
  EXPECT_DOUBLE_EQ(axis.y, 648);
  double theta = .6, radius = theta;
  for (size_t i = 0; i < 6; ++i)
    radius += camera.d[i] * std::pow(theta, i + 2);
  auto projected = camera.project(
      {std::sin(theta) / std::sqrt(2.), std::sin(theta) / std::sqrt(2.), std::cos(theta)});
  ...
  camera.d.clear();
  EXPECT_THROW(camera.validate(), std::invalid_argument);
}
 ...
 protected:
  ...
  void line(double offset, double curvature = 0) {
    for (int row = 0; row < gray.rows; ++row) {
      ...
      if (col >= 2 && col < gray.cols - 2)
        gray.row(row).colRange(col - 2, col + 2).setTo(0);
    }
  }
  Detection run() {
    return detect(gray, visible, grid, 65, .008, .05, .18, .12);
  }
};
...
TEST_F(LineTest, OffsetAndCurve) {
  ...
  for (auto p : result.points)
    EXPECT_NEAR(p.y, .025 + .15 * p.x * p.x, .008);
}
...
TEST_F(LineTest, ForkAndCrossingRejected) {
  ...
  gray.setTo(180);
  line(0);
  gray.rowRange(30, 35).setTo(0);
  // 全宽横线会被裁除，使观测路径在交叉处之前断开。
  EXPECT_FALSE(run().valid());
  gray.setTo(180);
  line(0);
  gray.rowRange(30, 35).colRange(45, 75).setTo(0);
  EXPECT_EQ(run().reason, "ambiguous_branches");
}

TEST_F(LineTest, ShortAndEdgeClippedRejected) {
  line(0);
  gray.rowRange(10, gray.rows).setTo(180);
  EXPECT_FALSE(run().valid());
  gray.setTo(180);
  gray.colRange(0, 4).setTo(0);
  EXPECT_FALSE(run().valid());
}

TEST_F(LineTest, DistantCrossingTruncatesWithoutSelectingBranch) {
  line(0);
  gray.rowRange(75, 80).colRange(45, 75).setTo(0);
  auto result = run();
  ASSERT_TRUE(result.valid());
...
TEST_F(LineTest, LeftAndRightCornersKeepLateralExit) {
  for (int sign : {-1, 1}) {
    ...
    EXPECT_NEAR(result.corner.x, .50, .015);
    EXPECT_NEAR(result.corner.y, 0, .015);
...
TEST_F(LineTest, ConnectedSCurveAndDisconnectedNoise) {
  ...
  for (int r = 0; r < grid.rows(); ++r)
    path.push_back({60 + int(12 * std::sin(r * .06)), r});
  ...
  for (size_t i = 1; i < result.points.size(); ++i)
    EXPECT_LT(cv::norm(result.points[i] - result.points[i - 1]), .008);
}

TEST_F(LineTest, InvisibleGapIsNotBridgedAndPriorCannotPickRemoteLine) {
  line(0);
  visible.rowRange(20, 24).setTo(0);
  EXPECT_FALSE(run().valid());
  visible.setTo(255);
...
TEST_F(LineTest, BlindBoundaryFragmentsNeverInventMultipleExits) {
  cv::line(gray, {15, 10}, {55, 8}, {0}, 4);
  cv::line(gray, {65, 8}, {105, 10}, {0}, 4);
  auto result = run();
  EXPECT_FALSE(result.valid());
  EXPECT_LT(result.exits, 2u);
...
TEST_F(LineTest, HintDirectionUsesSupportedChordNotPixelEdge) {
  ...
  for (int row = 2; row < 18; ++row) {
    auto point = grid.point(row, 0);
    point.y = .3 * point.x * point.x;
